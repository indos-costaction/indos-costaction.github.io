...
- `_data/members.yaml` is the canonical source for people in member and coordination roles. Coordinators have `coordination` and `coordination_order` fields. Pages such as `coordination.md`, `wgX.md`, `about.md`, and `training.md` render the shared people records with display options.
...
- The `person` include accepts a loop `person` object (or `name` for a direct lookup) and optional `coordination`, `photo`, `email`, `institution` (`"yes"`/`"no"`), plus `tag` (pipe-separated) and `class`. Prefer it over duplicating details that belong in `_data/members.yaml`.
